...
def loadtestData(config):
	test_image_ids = load_set(config['test_data'])
	# Load captions
	test_captions, _count = load_cleaned_captions(
		config['model_data_path']+'captions.txt', test_image_ids)
	# Load image features
	test_features = load_image_features(config['model_data_path']+'features_'+str(config['model_type'])+'.pkl', test_image_ids)
	print('{}: Available images for test: {}'.format(
		mytime(), len(test_features)))
	print('{}: Available captions for validation: {}'.format(mytime(), _count))
	return test_features, test_captions
